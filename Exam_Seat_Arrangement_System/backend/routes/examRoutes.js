...
import { createExam, deleteExam, getExamById, getExams, getExamsForInvigilator, updateExam } from '../controllers/examControllers.js';

const router = express.Router();

router.post('/add', authenticate, roleCheck(['ADMIN']), createExam);
router.get('/all', authenticate, roleCheck(['ADMIN']), getExams);
router.get('/assigned', authenticate, roleCheck(['INVIGILATOR']), getExamsForInvigilator);
router.get('/:id', authenticate, roleCheck(['ADMIN', 'INVIGILATOR']), getExamById);
