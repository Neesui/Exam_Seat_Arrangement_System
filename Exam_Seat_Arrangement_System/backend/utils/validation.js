...
export const validateCoursePayload = (name, duration, semesters, batchYear) => {
  if (!name || !name.trim()) return "Course name is required.";
  if (!duration || isNaN(duration)) return "Valid duration is required.";
  if (!batchYear || isNaN(batchYear)) return "Valid batchYear is required.";
